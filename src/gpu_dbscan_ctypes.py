...
def load_parameters():
        ...
        if arg == "--std_scale" and i + 1 < len(sys.argv):
            try:
                ...
                if std_scale < 0 or std_scale > 1:
                    print("gpu_dbscan_ctypes: std_scale must be between 0 and 1")
                    sys.exit(1)
                print(f"gpu_dbscan_ctypes: Using user-provided std_scale: {std_scale}")
            except ValueError:
                print("gpu_dbscan_ctypes: std_scale must be a float between 0 and 1")
                sys.exit(1)
            i += 2
            ...
            try:
                ...
                if min_pts < 1:
                    print("gpu_dbscan_ctypes: min_pts must be a positive integer")
                    sys.exit(1)
                print(f"gpu_dbscan_ctypes: Using user-provided min_pts: {min_pts}")
            except ValueError:
                print("gpu_dbscan_ctypes: min_pts must be a positive integer")
                sys.exit(1)
            i += 2
            ...
            try:
                ...
                if eps <= 0:
                    print("gpu_dbscan_ctypes: eps must be a positive float")
                    sys.exit(1)
                print(f"gpu_dbscan_ctypes: Using user-provided eps: {eps}")
            except ValueError:
                print("gpu_dbscan_ctypes: eps must be a positive float")
                sys.exit(1)
            i += 2
            ...
            print(f"gpu_dbscan_ctypes: Warning: Unknown argument {arg}")
            i += 1
        else:
    ...
    if input_filename is None:
        print("gpu_dbscan_ctypes: Error: Must specify an input file (image or netCDF)")
        print("Usage: python3 gpu_dbscan_ctypes.py <input_file> [--std_scale VALUE] [--min_pts VALUE] [--eps VALUE]")
        print("Example: python3 gpu_dbscan_ctypes.py imagen.jpg --min_pts 10")
        print("Example: python3 gpu_dbscan_ctypes.py imagen.jpg --std_scale 0.8 --min_pts 5")
        print("Example: python3 gpu_dbscan_ctypes.py datos.nc --eps 2.5")
        sys.exit(1)
...
def calculate_min_pts():
    ...
    print(f"gpu_dbscan_ctypes: Calculated min_pts as 2 * dimension + 1 => 2 * {dimension} + 1 = {calculated_min_pts}")
...
def load_data():
    ...
    if ext in [".jpg", ".jpeg", ".png", ".tif", ".tiff"]:
        ...
        print(f"gpu_dbscan_ctypes: Unsupported file extension: {ext}")
    ...
    if user_min_pts is not None:
        ...
        print(f"gpu_dbscan_ctypes: Using calculated min_pts: {min_pts}")
    
    # If std_scale not provided, use default value
    if std_scale is None:
        std_scale = 1.0
        print("gpu_dbscan_ctypes: Using default std_scale: 1.0")
...
def load_image(image_filename):
    ...
    print(f"gpu_dbscan_ctypes: Image name: {image_filename} Size: {image_orig.size}")
...
def get_epsilon(points, k,std_scale, user_eps=None):
    ...
    if user_eps is not None:
        print(f"gpu_dbscan_ctypes: Using user-provided epsilon: {user_eps}")
    ...
    print(f"gpu_dbscan_ctypes: Recommended epsilon: {epsilon[0]}")
    return epsilon
...
def dbscan(points, eps, min_pts,timeEpsilon):
    ...
    print("gpu_dbscan_ctypes: TimeGraph = ", timeGraph - timeEpsilon)
    ...
    print("gpu_dbscan_ctypes: TimeDBSCAN = ", time.perf_counter() - timeGraph)
...
def compute_cluster_properties(points, labels, cluster_count):
    ...
    if len(valid_labels) == 0:
        print("gpu_dbscan_ctypes: No clusters found for property computation.")
...
def save_cluster_properties(cluster_centers, cluster_radii, cluster_eigenvalues, 
    ...
    print(f"gpu_dbscan_ctypes: Cluster properties saved to {filename}")
...
def create_cluster_histograms(cluster_sizes, cluster_radii, cluster_eigenvalues_relation, 
    ...
    if len(sizes) == 0:
        print("gpu_dbscan_ctypes: No valid clusters for histogram creation.")
        return
    ...
    print(f"gpu_dbscan_ctypes: Histogram data saved to {data_filename}")
...
def plot_clusters(points, labels, cluster_count, input_filename):
    ...
    print(f"gpu_dbscan_ctypes: Cluster plot saved as: {output_filename}")
...
if __name__ == "__main__":
    print("===========================================")
    print("gpu_dbscan_ctypes: Starting GPU DBSCAN clustering")

    print("=== GPU Info ===")
    print(f"gpu_dbscan_ctypes: Number of devices: {cp.cuda.runtime.getDeviceCount()}")

    # Current device
    current_device = cp.cuda.runtime.getDevice()
    props = cp.cuda.runtime.getDeviceProperties(current_device)
    gpu_name = props['name'].decode()
    print(f"gpu_dbscan_ctypes: Current device: {current_device} - {gpu_name}")
    ...
    print(f"gpu_dbscan_ctypes: Number of points to cluster: {len(points_cpu)//2}")
    timePoints = time.perf_counter() # Time after points extraction
    print("gpu_dbscan_ctypes: TimePoints = ", timePoints - start)
    ...
    print("gpu_dbscan_ctypes: TimeEpsilon = ", timeEpsilon - timePoints)

    print("=== DBSCAN Clustering ===")
    labels,cluster_count = dbscan(points, eps, min_pts,timeEpsilon=timeEpsilon)
    print(f"gpu_dbscan_ctypes: Number of clusters found: {cluster_count}")
    timeDBSCAN = time.perf_counter()
    print("gpu_dbscan_ctypes: Time DBSCAN and graph construction = ", timeDBSCAN - timeEpsilon)
    ...
    print("gpu_dbscan_ctypes: Time Properties = ", timeProperties - timeDBSCAN)
    
    # End timing
    end = time.perf_counter()
    print("gpu_dbscan_ctypes: Total Time = ", end - start)
    
    print("gpu_dbscan_ctypes: Finished clustering and property computation. Saving results...")
