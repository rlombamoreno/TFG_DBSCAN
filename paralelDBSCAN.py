...
def get_points_in_cluster(image, color_marker):
    ...
    kernel_code = r'''
    extern "C" __global__
    void get_points_in_cluster(const int *image, int *points, int color_marker, int y_len, int x_len, int *points_index) {
...
def count_cluster_points(image, color_marker, y_len, x_len):
    ...
    kernel_code =  r'''
    extern "C" __global__
    void count_points(const int *image, int *count, int color_marker, int y_len, int x_len) {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < y_len * x_len) {
            int x = idx % x_len;
            int y = idx / x_len;
...
def compute_kn_distances(points, k):
    ...
    kernel_code =  r'''
    extern "C" __global__
    void compute_kn_distances(const int *points, double *kn_distances, int num_points, int k) {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < num_points) {
            int x1 = points[idx * 2];
